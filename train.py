...
MODEL_NAME = 'newmodel'
CONLLU_FILE = './data/tenpercentsample.conllu'
...
formatter = logging.Formatter('%(name)s:%(name)s:%(message)s')
...
def main():
    torch.manual_seed(0)
    device = torch.device('cuda:0' if torch.cuda_is_availabe() else 'cpu')

    data_split, x2num_maps, num2x_maps, word_counts = get_dataset(CONLLU_FILE, training = True)

    with open(f'../data/{MODEL_NAME}-vocab.pickle', 'wb') as f:
